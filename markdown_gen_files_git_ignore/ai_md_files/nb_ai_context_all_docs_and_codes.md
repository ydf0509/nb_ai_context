...
- `def add_ai_reading_guide(self, guide_lang = 'cn') -> 'AiMdGenerator'`
  - **Docstring:**
  `````
...
**Class Variables (1):**
- `suffix__lang_map = {'.py': 'python', '.md': 'markdown', '.txt': 'text', '.json': 'json', '.yaml': 'yaml', '.yml': 'yaml', '.xml': 'xml', '.html': 'html', '.css': 'css', '.js': 'javascript', '.ts': 'typescript', '.jsx': 'javascript', '.tsx': 'typescript', '.vue': 'vue', '.php': 'php', '.java': 'java', '.c': 'c', '.cpp': 'cpp', '.h': 'h', '.hpp': 'hpp', '.cs': 'csharp', '.vb': 'vb', '.sql': 'sql', '.bat': 'batch', '.sh': 'shell', '.ps1': 'powershell', '.psm1': 'powershell', '.psd1': 'powershell', '.pssc': 'powershell', '.psscx': 'powershell'}`
...
ai_guide_en = '''
# 🤖 AI Context Protocol (Generated by nb_ai_context)

> **SYSTEM INSTRUCTION**: You are processing a **structured knowledge snapshot** automatically generated by the tool **`nb_ai_context`**.
> **NATURE OF DOCUMENT**: This is **NOT** a standard human-readable document. It is a machine-generated stream that merges project documentation, source code, and AST metadata specifically for LLM analysis.

## 🧠 Cognitive Guidelines for AI

This document serves as your injected knowledge base. The content provided by `nb_ai_context` is flexible—it may contain comprehensive source code, only documentation/tutorials, or a specific subset of core files. Please adopt the following adaptive reading strategies:

### 1. Hierarchy of Information
*   **Documentation as Intent**: Treat `README`, tutorials, and docstrings as the high-level **Source of Truth** for *how* the project is intended to be used. If full source code is missing (omitted by the generator), rely on these descriptions to understand the logic and flow.
*   **Source Code & AST as Reality**: When `.py` files or AST Metadata (class signatures) are present, use them to verify implementation details, strict typing, and internal mechanics.
*   **Bridging the Gap**: If you see a tutorial explaining a feature, but the corresponding source code is not included in this snapshot, **do not assume it is missing or broken**. Instead, infer the API's behavior and existence from the documentation's usage examples.

### 2. Understanding File Boundaries & Structure
*   **File Context**: `nb_ai_context` uses strict delimiters to mark file boundaries:
    `--- **start of file: <path>** ---` ... content ... `--- **end of file: <path>** ---`
*   **Relative Paths**: All paths correspond to the actual project structure. Use the "File Tree" section to visualize the architecture, even for files that are not fully expanded in the text.
*   **Imports & Dependencies**: Use the "File Dependencies Analysis" section (generated by the tool) to understand how modules connect, especially when analyzing partial codebases.

### 3. Code Generation & Explanation
*   **Context-Aware Coding**: When asked to generate code, align your style with the provided examples. If only tutorials are available, write code that fits the "public API" described in the text.
*   **AST Utilization**: For Python files where `nb_ai_context` has extracted only AST metadata (signatures/classes), trust these signatures for method calls and type hinting, treating them as valid, existing interfaces.
*   **Fact Anchoring**: Generated code must be strictly **anchored** within the scope of this document.
    *   API invocations must be based on either **AST signatures in source code** or **usage examples in tutorials**.
    *   **Strictly Forbid Fabrication**: You are strictly prohibited from inventing class names, method names, or parameters that are neither defined in the source definitions nor mentioned in the tutorials. Ensure that every generated token has a textual basis in this document.
...
class AiMdGenerator(NbPath):
    ...
    def add_ai_reading_guide(self,guide_lang="cn") -> "AiMdGenerator":
        ...
        guide = ai_guide_en if guide_lang == "en" else ai_guide_cn
    ...
    def merge_from_dir(
        ...
        # 打印体积最大的20个文件
        if relative_paths_to_include:
            file_sizes = []
            for rel_path in relative_paths_to_include:
                abs_path = project_root_path / rel_path
                try:
                    size = abs_path.stat().st_size
                    file_sizes.append((rel_path, size))
                except Exception:
                    file_sizes.append((rel_path, 0))
            
            # 按文件大小降序排序
            file_sizes.sort(key=lambda x: x[1], reverse=True)
            
            # 打印体积最大的20个文件
            print(f"\n📊 Top 20 largest files in '{relative_dir_name}' (total: {len(file_sizes)} files):")
