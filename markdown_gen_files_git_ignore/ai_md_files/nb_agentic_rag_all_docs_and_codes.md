...
---
# markdown content namespace: nb_agentic_rag project summary 



- `nb_agentic_rag` 是一个 Agentic RAG MCP Server，让 AI Agent 自主决定何时搜、搜什么、搜几次。
- 核心特点：12 个精心设计的互补 MCP 工具，吊打传统一轮检索的 Naive RAG。
- 核心模块：
  - `nb_agentic_rag/server.py`: FastMCP 服务端，定义 12 个 MCP 工具
  - `nb_agentic_rag/core.py`: RAG 核心逻辑（ChromaDB 向量存储 + SiliconFlow Embedding/Rerank + 原文缓存）
  - `nb_agentic_rag/chunker.py`: 文本分块 + Python AST scope 注入
  - `nb_agentic_rag/config.py`: 三层配置加载（CLI > ENV > YAML）
- 工具类别：导入(ingest)、语义检索(search)、精确检索(grep/find_definition)、上下文扩展(get_file/adjacent/lines)、管理(list/delete/stats)
- 支持 stdio / streamable-http / SSE 三种传输模式
- 可通过 `uvx nb_agentic_rag` 或 `pip install nb_agentic_rag` 安装


## 📋 nb_agentic_rag most core source files metadata (Entry Points)


以下是项目 nb_agentic_rag 最核心的入口文件的结构化元数据，帮助快速理解项目架构：



### the project nb_agentic_rag most core source code files as follows: 
- `nb_agentic_rag/server.py`


### 📄 Python File Metadata: `nb_agentic_rag/server.py`

#### 📝 Module Docstring

`````
nb_agentic_rag MCP Server — 12 个 Agentic RAG 工具。
...
启动方式:
  uvx nb_agentic_rag                          # stdio (默认)
  uvx nb_agentic_rag --transport streamable-http --port 9101  # HTTP
`````
...
- `from nb_agentic_rag.config import get_config`
- `from nb_agentic_rag.chunker import DEFAULT_CHUNK_SIZE`
- `from nb_agentic_rag.chunker import DEFAULT_CHUNK_OVERLAP`
- `from nb_agentic_rag.core import ingest_path`
- `from nb_agentic_rag.core import search`
- `from nb_agentic_rag.core import list_documents`
- `from nb_agentic_rag.core import delete_document`
- `from nb_agentic_rag.core import get_stats`
- `from nb_agentic_rag.core import get_file_chunks`
- `from nb_agentic_rag.core import get_context_chunks`
- `from nb_agentic_rag.core import grep_knowledge`
- `from nb_agentic_rag.core import find_symbol_definition`
- `import argparse`
- `from nb_agentic_rag.config import load_config`

#### 🔧 Public Functions (13)

- `def rag_add_document(path: str = Field(description='Absolute path to a file or directory. Directory imports all text files recursively'), collection_name: str = Field(description='Collection name (required, auto-created if not exists. Use rag_stats to see existing)'), chunk_size: int = Field(default=DEFAULT_CHUNK_SIZE, description='Chunk size in chars, default 1500, recommended 1000-2000'), chunk_overlap: int = Field(default=DEFAULT_CHUNK_OVERLAP, description='Chunk overlap in chars, default 200'), file_extensions: str = Field(default='', description="Comma-separated file extensions to include (e.g. 'py,md,html'). Empty = all text files")) -> str` `mcp.tool()`
  - *Line: 36*
  - **Docstring:**
  `````
  ...
  Use rag_search to retrieve ingested content.
  Note: This tool is typically called manually by users via scripts for one-time batch ingestion,
  not by AI during conversations. AI should focus on rag_search/rag_grep for retrieval.
  `````

- `def rag_search(query: str = Field(description='Search query (natural language question or keywords)'), collection_name: str = Field(description='Collection name (required, use rag_stats to see available collections)'), top_k: int = Field(default=5, description='Number of results to return'), use_rerank: bool = Field(default=True, description='Enable reranker for better accuracy (recommended)'), filter_filename: str = Field(default='', description="Optional: filter by filename only (e.g. 'constant.py', not full path)")) -> str` `mcp.tool()`
  - *Line: 76*
  - **Docstring:**
  `````
...
- `def rag_grep(keyword: str = Field(description="Keyword or regex pattern (e.g. class name 'UserService', method 'get_by_id', constant 'MAX_RETRIES')"), collection_name: str = Field(description='Collection name (required, use rag_stats to see available collections)'), max_results: int = Field(default=10, description='Maximum number of matches to return'), case_sensitive: bool = Field(default=False, description='Case-sensitive matching'), filter_filename: str = Field(default='', description="Optional: filter by filename only (e.g. 'booster.py', not full path)"), context_lines: int = Field(default=10, description='Lines of context before and after each match (default 10, use 20+ for seeing full function/class headers)')) -> str` `mcp.tool()`
  - *Line: 270*
  - **Docstring:**
  `````
...
- `def rag_find_definition(symbol: str = Field(description="Symbol name to find (class name like 'UserService', function name like 'get_by_id', or qualified name like 'MyClass.__init__')"), collection_name: str = Field(description='Collection name (required, use rag_stats to see available collections)'), max_results: int = Field(default=5, description='Maximum number of definitions to return')) -> str` `mcp.tool()`
  - *Line: 309*
  - **Docstring:**
  `````
...
- `def rag_get_file_chunks(file_path: str = Field(description="Full absolute path (use 'file_path' value from rag_search results)"), collection_name: str = Field(description='Collection name (use rag_stats to see available collections)'), start_chunk: int = Field(default=0, description='Start chunk index (0-based), use for pagination'), max_chunks: int = Field(default=10, description='Max chunks to return (use for pagination)')) -> str` `mcp.tool()`
  - *Line: 349*
  - **Docstring:**
  `````
...
- `def rag_get_raw_file(file_path: str = Field(description="Full absolute path (use 'file_path' value from rag_search results)"), collection_name: str = Field(description='Collection name (use rag_stats to see available collections)'), line_start: int = Field(default=-1, description='Start line (1-based), -1 for beginning'), line_end: int = Field(default=-1, description='End line (inclusive), -1 for end of file')) -> str` `mcp.tool()`
  - *Line: 369*
  - **Docstring:**
  `````
...
- `def rag_get_adjacent_chunks(doc_id: str = Field(description="Document ID (use 'doc_id' value from rag_search/rag_grep results)"), chunk_index: int = Field(description="Target chunk index (0-based, the X from 'chunk:X/Y' in rag_search results)"), collection_name: str = Field(description='Collection name (use rag_stats to see available collections)'), window: int = Field(default=3, description='Context window: returns chunk_index ± window chunks')) -> str` `mcp.tool()`
  - *Line: 437*
  - **Docstring:**
  `````
...
- `def rag_get_chunks_by_lines(doc_id: str = Field(description="Document ID (use 'doc_id' value from rag_search/rag_grep results)"), line_start: int = Field(description="Start line number (1-based, from 'line:N-M' in search results)"), line_end: int = Field(description="End line number (inclusive, from 'line:N-M' in search results)"), collection_name: str = Field(description='Collection name (use rag_stats to see available collections)')) -> str` `mcp.tool()`
  - *Line: 454*
  - **Docstring:**
  `````
...
- `def rag_list(collection_name: str = Field(description='Collection name (required, use rag_stats to see available collections)')) -> str` `mcp.tool()`
  - *Line: 497*
  - **Docstring:**
  `````
...
- `def rag_delete(doc_id: str = Field(description="Document ID to delete (use 'doc_id' value from rag_list results)"), collection_name: str = Field(description='Collection name (use rag_stats to see available collections)')) -> str` `mcp.tool()`
  - *Line: 516*
  - *Delete a document's vectors and cached files from the collection. Get doc_id from rag_list.*

- `def rag_stats() -> str` `mcp.tool()`
  - *Line: 530*
  - **Docstring:**
  `````
...
- `def main()`
  - *Line: 560*
  - *CLI entry point for uvx / python -m nb_agentic_rag.*
...
## 🔗 nb_agentic_rag Some File Dependencies Analysis
...
---
# markdown content namespace: nb_agentic_rag Project Root Dir Some Files 


## nb_agentic_rag File Tree (relative dir: `.`)
...
## nb_agentic_rag (relative dir: `.`)  Included Files (total: 2 files)
...
--- **start of file: README.md** (project: nb_agentic_rag) --- 
...
# 或 python 方式
python -m nb_agentic_rag --transport streamable-http --port 9101
```
...
```python
from nb_agentic_rag.core import batch_ingest
...
Skill 内容包括：发现（rag_stats）→ 检索（4 种策略选择）→ 深入（4 种上下文获取方式）→ 多轮重试策略。
...
有了这些 metadata + 头部注入，`rag_search` 搜 "process 方法" 就能精准命中 `MyClass.process`，而不是随机匹配到别的 "process" 字符串。
...
| 变量 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `NB_RAG_API_KEY` | **是** | | Embedding/Rerank API Key |
| `NB_RAG_BASE_URL` | 否 | `https://api.siliconflow.cn/v1` | API Base URL |
| `NB_RAG_EMBEDDING_MODEL` | 否 | `BAAI/bge-m3` | Embedding 模型 |
| `NB_RAG_RERANK_MODEL` | 否 | `BAAI/bge-reranker-v2-m3` | Rerank 模型 |
| `NB_RAG_DB_PATH` | 否 | `./rag_db` | ChromaDB 存储路径 |
| `NB_RAG_CHUNK_SIZE` | 否 | `1500` | 分块大小 |
| `NB_RAG_CHUNK_OVERLAP` | 否 | `200` | 分块重叠 |
...
即使 chunk 切分不完美，Agentic RAG 也不会像 Naive RAG 那样只用碎片凑答案。AI 会自主判断当前 chunk 信息不足，然后调用 `rag_get_raw_file` 读完整源码、用 `rag_find_definition` 精确定位类/函数定义、用 `rag_grep` 全文搜索关键词，多轮组合直到信息充分。这正是 "检索是 Agent 的能力" 而非固定管道的意义。
...
--- **end of file: README.md** (project: nb_agentic_rag) --- 

---


--- **start of file: pyproject.toml** (project: nb_agentic_rag) --- 
...
[project.urls]
Homepage = "https://github.com/ydf0509/nb_agentic_rag"
Repository = "https://github.com/ydf0509/nb_agentic_rag"

[project.scripts]
nb_agentic_rag = "nb_agentic_rag.server:main"
...
--- **end of file: pyproject.toml** (project: nb_agentic_rag) --- 

---

# markdown content namespace: nb_agentic_rag codes 


## nb_agentic_rag File Tree (relative dir: `nb_agentic_rag`)
...
## nb_agentic_rag (relative dir: `nb_agentic_rag`)  Included Files (total: 7 files)


- `nb_agentic_rag/chunker.py`

- `nb_agentic_rag/config.py`

- `nb_agentic_rag/core.py`

- `nb_agentic_rag/loggers.py`

- `nb_agentic_rag/server.py`

- `nb_agentic_rag/__init__.py`

- `nb_agentic_rag/__main__.py`


---


--- **start of file: nb_agentic_rag/chunker.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/chunker.py`
...
--- **end of file: nb_agentic_rag/chunker.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/config.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/config.py`
...
def _find_config_file():
    """按优先级查找配置文件。"""
    candidates = [
        os.path.join(os.getcwd(), "nb_rag_config.yaml"),
        os.path.join(os.getcwd(), "nb_rag_config.yml"),
        os.path.expanduser("~/.config/nb_agentic_rag/config.yaml"),
        os.path.expanduser("~/.config/nb_agentic_rag/config.yml"),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return c
...
def load_config(cli_args=None) -> RagConfig:
    ...
    if cli_args and hasattr(cli_args, 'config') and cli_args.config:
        ...
        yaml_path = os.environ.get("NB_RAG_CONFIG", None) or _find_config_file()
    ...
    api_key = (
        ...
        or os.environ.get("NB_RAG_API_KEY", "")
        or _resolve_env_ref(embedding_data.get("api_key", ""))
    )

    base_url = (
        os.environ.get("NB_RAG_BASE_URL", "")
        or embedding_data.get("base_url", "")
        or "https://api.siliconflow.cn/v1"
    )

    embedding_model = (
        os.environ.get("NB_RAG_EMBEDDING_MODEL", "")
        or embedding_data.get("model", "")
        or "BAAI/bge-m3"
    )

    rerank_model = (
        os.environ.get("NB_RAG_RERANK_MODEL", "")
    ...
    db_path = (
        ...
        or os.environ.get("NB_RAG_DB_PATH", "")
        or storage_data.get("db_path", "")
        or "./rag_db"
    )

    raw_files_path = (
        os.environ.get("NB_RAG_RAW_FILES_PATH", "")
        or storage_data.get("raw_files_path", "")
        or ""
    )

    chunk_size = int(
        os.environ.get("NB_RAG_CHUNK_SIZE", "0")
        or chunking_data.get("chunk_size", 0)
        or 1500
    )

    chunk_overlap = int(
        os.environ.get("NB_RAG_CHUNK_OVERLAP", "0")
...
--- **end of file: nb_agentic_rag/config.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/core.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/core.py`
...
- `import chromadb`
- `from nb_agentic_rag.config import get_config`
- `from nb_agentic_rag.chunker import chunk_text`
- `from nb_agentic_rag.chunker import enrich_chunks`
- `from nb_agentic_rag.chunker import collect_files`
- `from nb_agentic_rag.chunker import DEFAULT_CHUNK_SIZE`
- `from nb_agentic_rag.chunker import DEFAULT_CHUNK_OVERLAP`
...
- `from nb_agentic_rag.chunker import _extract_signature`
...
import chromadb
from nb_agentic_rag.config import get_config
from nb_agentic_rag.chunker import (
...
def embed(texts, max_retries=3):
    ...
    if not api_key:
        raise ValueError(
            "NB_RAG_API_KEY is empty. Set the environment variable or configure in nb_rag_config.yaml"
        )
    import time as _time
...
--- **end of file: nb_agentic_rag/core.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/loggers.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/loggers.py`
...
logger = nb_log.get_logger("nb_agentic_rag")
`````

--- **end of file: nb_agentic_rag/loggers.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/server.py** (project: nb_agentic_rag) --- 
...
from mcp.server.fastmcp import FastMCP
from nb_agentic_rag.config import get_config
from nb_agentic_rag.chunker import DEFAULT_CHUNK_SIZE, DEFAULT_CHUNK_OVERLAP
from nb_agentic_rag.core import (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="nb_agentic_rag — Agentic RAG MCP Server with 12 tools",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""Examples:
  nb_agentic_rag                               # stdio transport (default)
  nb_agentic_rag --transport streamable-http    # HTTP transport on port 9101
  nb_agentic_rag --api-key sk-xxx              # specify API key directly
  NB_RAG_API_KEY=sk-xxx nb_agentic_rag         # API key via env var""",
    )
    ...
    parser.add_argument("--api-key", dest="api_key",
                        help="Embedding/Rerank API key (or set NB_RAG_API_KEY env var)")
...
--- **end of file: nb_agentic_rag/server.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/__init__.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/__init__.py`

#### 📝 Module Docstring

`````
nb_agentic_rag — Agentic RAG MCP Server
...
--- **end of file: nb_agentic_rag/__init__.py** (project: nb_agentic_rag) --- 

---


--- **start of file: nb_agentic_rag/__main__.py** (project: nb_agentic_rag) --- 


### 📄 Python File Metadata: `nb_agentic_rag/__main__.py`

#### 📝 Module Docstring

`````
python -m nb_agentic_rag 入口。
`````

#### 📦 Imports

- `from nb_agentic_rag.server import main`
...
--- **end of file: nb_agentic_rag/__main__.py** (project: nb_agentic_rag) --- 

---

# markdown content namespace: nb_agentic_rag scripts 


## nb_agentic_rag File Tree (relative dir: `scripts`)
...
## nb_agentic_rag (relative dir: `scripts`)  Included Files (total: 4 files)
...
--- **start of file: scripts/ingest_funboost.py** (project: nb_agentic_rag) --- 
...
--- **end of file: scripts/ingest_funboost.py** (project: nb_agentic_rag) --- 

---


--- **start of file: scripts/ingest_funboost_one_md.py** (project: nb_agentic_rag) --- 
...
--- **end of file: scripts/ingest_funboost_one_md.py** (project: nb_agentic_rag) --- 

---


--- **start of file: scripts/ingest_project.py** (project: nb_agentic_rag) --- 
...
--- **end of file: scripts/ingest_project.py** (project: nb_agentic_rag) --- 

---


--- **start of file: scripts/start_http_rag_mcp.py** (project: nb_agentic_rag) --- 

`````python
"""
启动 nb_agentic_rag HTTP MCP Server。

    set NB_RAG_API_KEY=sk-xxx
...
from nb_agentic_rag.config import load_config
from nb_agentic_rag.server import mcp
...
print(f"[nb_agentic_rag] HTTP MCP server on http://localhost:{PORT}/mcp")

mcp.run(transport="streamable-http")

`````

--- **end of file: scripts/start_http_rag_mcp.py** (project: nb_agentic_rag) --- 

---

# markdown content namespace: nb_agentic_rag skills 


## nb_agentic_rag File Tree (relative dir: `skills/rag-workflow`)
...
## nb_agentic_rag (relative dir: `skills/rag-workflow`)  Included Files (total: 1 files)


- `skills/rag-workflow/SKILL.md`


---


--- **start of file: skills/rag-workflow/SKILL.md** (project: nb_agentic_rag) --- 

`````markdown
---
name: rag-workflow
description: >-
  使用 nb_agentic_rag 知识库进行多轮智能检索。当用户询问代码实现、
  查找函数定义、理解项目结构、搜索文档内容、或提到"知识库""搜索代码"时使用。
---

# RAG 知识库检索工作流

nb_agentic_rag 提供 12 个 MCP 工具，核心分三类：**发现 → 检索 → 深入**。
所有工具返回格式化文本（非 JSON），需自行解析。

## 前提

- 以下 4 个工具依赖导入时的 raw cache：`rag_grep`、`rag_find_definition`、`rag_get_raw_file`、`rag_search_and_fetch`。
  - `rag_get_raw_file` 缺 cache 时报：`Raw file cache not found (doc_id=...). This file may have been imported before caching was enabled. Please re-import.`
  - `rag_search_and_fetch` 缺 cache 时报：`[{file_path}] raw cache not available`
  - `rag_grep` / `rag_find_definition` 缺 cache 时**静默返回空结果**（不报错），表现与"真的没有匹配"相同——如果怀疑是 cache 缺失，先用 `rag_stats` 确认 collection 存在且有 chunks。
...
注意：仅 `rag_search` / `rag_search_and_fetch` 在 collection 不存在时会提示 `Available collections: [...]`，
`rag_grep` / `rag_find_definition` 不会提示（静默返回空）。
...
```
rag_search(query="用户的问题", collection_name="xxx", top_k=5)
```
...
```
rag_grep(keyword="UserService", collection_name="xxx")
```
...
```
rag_find_definition(symbol="get_by_id", collection_name="xxx")
```
...
```
rag_search_and_fetch(query="...", collection_name="xxx", top_k=5)
```

- 语义搜索 + 自动抓取 top N 结果的 raw 源码（省去单独调用 `rag_get_raw_file`）
...
| 工具 | 特点 | 适用场景 |
|------|------|---------|
| `rag_get_raw_file` | 无 overlap 的干净源码，无 scope | 看完整文件或指定行号范围 |
| `rag_get_file_chunks` | 有 scope 元数据，chunk 间有 200 字符 overlap | 分页浏览 + 了解作用域 |
| `rag_get_adjacent_chunks` | 有 scope，有 overlap | 扩展搜索结果的上下文 |
| `rag_get_chunks_by_lines` | 有 scope，有 overlap | 按行号精确取 chunk |

### 看原始文件

```
rag_get_raw_file(file_path="从搜索结果获取", collection_name="xxx")
```
...
```
rag_get_adjacent_chunks(doc_id="从搜索结果获取", chunk_index=3, collection_name="xxx", window=3)
```
...
```
rag_get_chunks_by_lines(doc_id="...", line_start=50, line_end=100, collection_name="xxx")
```

### 分页浏览文件

```
rag_get_file_chunks(file_path="...", collection_name="xxx", start_chunk=0, max_chunks=10)
```

- 与 `rag_get_raw_file` 的区别：有 scope 元数据 + chunk 间有 overlap，适合需要作用域信息的场景
...
rag_stats()                          # 1. 发现知识库
    ↓
rag_search() / rag_grep()            # 2. 检索（可多次、换策略）
    ↓                                # 快捷路径：直接 rag_search_and_fetch() 一步到位
rag_find_definition()                # 3. 需要完整 class/function 定义时（Python 最佳）
    ↓
rag_get_raw_file()                   # 4. 看完整源码（无 overlap，无 scope）
或 rag_get_adjacent_chunks()          #    或扩展上下文（有 scope）
或 rag_get_chunks_by_lines()          #    或按行号取 chunk（有 scope）
或 rag_get_file_chunks()              #    或分页浏览全文件 chunks（有 scope）
    ↓
跨文件追踪：遇到未知符号 → 重复 2-4
```
...
1. **换关键词**：用同义词或更具体/更宽泛的词重新 `rag_search`
2. **换策略**：语义搜索不行就试 `rag_grep` 精确搜索
3. **缩小范围**：用 `filter_filename` 限定文件（仅 `rag_search` / `rag_grep` 支持）
4. **查定义**：找到了引用但不知道定义在哪 → `rag_find_definition`
...
| 工具 | 返回信息 |
|------|---------|
| `rag_search` / `rag_search_and_fetch` | `collection 'xxx' does not exist. Available collections: [...] Use rag_add_document to create and import docs.` |
| `rag_grep` / `rag_find_definition` | **静默返回空**（与"无匹配"不可区分） |

→ 建议：用 `rag_stats()` 确认 collection 名称。

### Collection 为空

| 工具 | 返回信息 |
|------|---------|
| `rag_search` / `rag_search_and_fetch` | `collection 'xxx' is empty. Use rag_add_document to import docs first.` |

### Raw cache 缺失

| 工具 | 返回信息 |
|------|---------|
| `rag_get_raw_file` | `Raw file cache not found (doc_id=...). This file may have been imported before caching was enabled. Please re-import.` |
| `rag_search_and_fetch` | `[{file_path}] raw cache not available` |
| `rag_grep` / `rag_find_definition` | **静默返回空** |
...
| `No definition found ... Try rag_grep` | 符号未找到（或 AST 解析失败） | 用 `rag_grep` 扩大搜索 |
| `File not found: 'xxx'` | 文件路径或文件名不存在 | 检查路径是否正确 |
| `Document not found: 'xxx'` | 文档 ID 不存在 | 用 `rag_list` 查看有效 doc_id |
| `No chunks matching line range ...` | 行号范围内无 chunk | 调整行号或用 `rag_get_raw_file` |
| `[path] excerpt failed` | 自动抓取源码失败 | 改用 `rag_get_raw_file` 手动抓取 |

## 不需要 AI 调用的工具

以下工具通常由用户手动操作：

- `rag_add_document` — 知识库导入（人工一次性批量操作）
- `rag_delete` — 删除文档
- `rag_list` — 列出文档（除非用户明确要求）

`````

--- **end of file: skills/rag-workflow/SKILL.md** (project: nb_agentic_rag) --- 
