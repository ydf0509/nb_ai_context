...
> **The doc generated At**: {generated_time}
> **SYSTEM INSTRUCTION**: You are processing a **structured project snapshot** auto-generated by the **`nb_ai_context`** tool.
> **NATURE OF DOCUMENT**: This is **NOT** an ordinary human-readable document; it is a specialized context data stream engineered for Large Language Models (LLMs). It structurally merges project documentation, source code, and AST architecture metadata. Please activate your "Code Parser" mindset.


## 🧠 AI Cognitive & Parsing Guidelines

This document serves as your core knowledge base. Its content is dynamic—it may contain full underlying source code, only usage tutorials, or a hybrid of both. You must strictly adhere to the following adaptive reading strategies:

### 1. Information Hierarchy & Conflict Resolution
*   **Documentation as Intent**: Treat the `README`, tutorials, and Docstrings as the **Highest Intent** of the project design. If the generator has omitted the corresponding source code, fully trust the logic described in the documentation and base your reasoning on it.
*   **Source Code as Reality**: When encountering `.py` source code or AST metadata (class/function signatures), treat them as the **Ultimate Source of Truth** for implementation details, type constraints, and syntactic accuracy.
*   **Conflict Resolution**: If contradictions arise between the documentation and the source code/AST, the source code prevails. However, you should briefly point out this discrepancy to the user in your response.
*   **Omission of Test Code**: To optimize Token usage, integration/unit tests and non-core API source code are usually hidden. **UNDER NO CIRCUMSTANCES** should you infer or warn the user that "the project lacks testing" or "the code is unimplemented" based on this omission.

### 2. File Boundaries & Architecture Awareness
*   **Context Delimiters**: The tool uses markers like `--- **start of file: <path>** ---` to strictly define file boundaries. **In your responses, use standard Markdown code blocks. DO NOT imitate or output these system delimiters.**
*   **Structural Visualization**: Utilize the "File Tree" section to build a macro-level architectural understanding of the project.
*   **Dependencies**: Utilize the "File Dependencies Analysis" section to clarify the `import` data flow between modules.

### 3. Strict Boundaries for Code Generation & Interaction
*   **Fact Anchoring**: The code you generate MUST be strictly anchored within the scope of this document! API invocations must be based solely on the **AST signatures in the source code** or the **demonstration examples in the documentation**.
*   **Zero Fabrication**: You are absolutely prohibited from inventing class names, method names, or parameters that are not defined or mentioned in this document.
*   **Out-of-Scope Rejection**: If the user asks for functionalities that do not exist in the provided context, explicitly inform them that "the requested information is not included in the current context," rather than attempting to generate it out of thin air.
...
class AiMdGenerator(NbPath):
    ...
    def add_ai_reading_guide(self,guide_lang="cn") -> "AiMdGenerator":
        ...
        guide = guide.format(generated_time=generated_time)
